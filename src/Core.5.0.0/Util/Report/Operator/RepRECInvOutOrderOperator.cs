...
{
    public class RepRECInvOutOrderOperator : RepTemplate1
    {
        public RepRECInvOutOrderOperator()
        {
            //明细部分的行数
        ...
        protected override bool FillValuesImpl(String templateFileName, IList<object> list)
        {
            try
            {
                ...
                foreach (PrintReceiptDetail receiptDetail in receiptDetailList)
                {
                    ...
                    this.SetRowCell(pageIndex, rowIndex, 7, receivedQty.ToString("0.########"));
                    //实收数  包装
                    //decimal UC = receiptDetail.UnitCount > 0 ? receiptDetail.UnitCount : 1;
                    //int UCs = (int)Math.Ceiling(receivedQty / UC);
                    this.SetRowCell(pageIndex, rowIndex, 6, receiptDetail.BoxQty);
        ...
        private void FillHead(PrintReceiptMaster receiptMaster, PrintReceiptDetail receiptDetail)
        {
            ...
            //Order No.:
            this.SetRowCell(2, 5, receiptMaster.ReceiptNo);
            //制单时间???
            this.SetRowCell(4, 2, receiptMaster.CreateDate.ToString("yyyy-MM-dd HH:mm:ss"));
            //领料时间
