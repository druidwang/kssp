...
namespace com.Sconit.Web.Controllers.SCM
{
    public class ProductionFlowController : WebAppBaseController
    {
        ...
        {
            string whereStatement = " where f.Type=4 ";
        ...
        [SconitAuthorize(Permissions = "Url_ProductionFlow_View")]
         ...
         {
             ViewBag.ProductLine = ProductLine;
             IList<ProductLineFacility> ProductLineFacilityList = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.ProductLine=?", ProductLine);
            if(!string.IsNullOrEmpty(code)){
                ProductLineFacilityList = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.Code=? and p.ProductLine=?", new object[] {code
                ,ProductLine}) ;
            }
         ...
         public ActionResult _InsertFacility(string id, bool IsActive, string ProductLine)
         {
             if (ModelState.IsValid)
             {
                 IList<ProductLineFacility> ProductLineFacilityByCode = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.Code=?", id); ;
                 if (ProductLineFacilityByCode.Count > 0)
                 {
                     SaveSuccessMessage(Resources.EXT.ControllerLan.Con_EquipmentAlreadyExistsRefill);
                 }
                 else
                 {
                     ...
                     genericMgr.CreateWithTrim(ProductLineFacility);
                     SaveSuccessMessage(Resources.EXT.ControllerLan.Con_EquipmentAddedSuccessfully);
                 }
             }
             IList<ProductLineFacility> ProductLineFacilityList = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.ProductLine=?", ProductLine);
             return PartialView(new GridModel(ProductLineFacilityList));
         ...
         public ActionResult _DeleteFacility(string id, string ProductLine)
         {
             ...
             genericMgr.DeleteById<ProductLineFacility>(id);
             SaveSuccessMessage(Resources.EXT.ControllerLan.Con_EquipmentDeletedSuccessfully);
             IList<ProductLineFacility> ProductLineFacilityList = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.ProductLine=?", ProductLine);
             return PartialView(new GridModel(ProductLineFacilityList));
         }
         ...
         public ActionResult _UpdateFacility(string id, bool IsActive)
         {
             ...
             ProductLineFacility ProductLineFacility = genericMgr.FindById<ProductLineFacility>(id);
             ProductLineFacility.IsActive = IsActive;
             genericMgr.UpdateWithTrim(ProductLineFacility);
             SaveSuccessMessage(Resources.EXT.ControllerLan.Con_EquipmentModificateSuccessfully);
             IList<ProductLineFacility> ProductLineFacilityList = genericMgr.FindAll<ProductLineFacility>("from ProductLineFacility as p where p.ProductLine=?", ProductLineFacility.ProductLine);
             return PartialView(new GridModel(ProductLineFacilityList));
         }
         ...
         public ActionResult Import(IEnumerable<HttpPostedFileBase> flowattachments)
         {
             try
             {
                 ...
                 {
                     flowMgr.ImportFlow(file.InputStream, CodeMaster.OrderType.Production);
                 }
         ...
         public ActionResult Export(FlowSearchModel searchModel)
         {
             ...
             return new DownloadFileActionResult(table, "ProductionFlowDetail.xls");
         }
         #endregion
         ...
         private string GetStringFlowInforView(IList<FlowDetail> flowDetails, bool isPercentage, bool isCss = false)
         {
             ...
             str = str.Append(Str(Resources.SCM.FlowMaster.FlowMaster_OrderDeviation, true, false, l));
             str = str.Append(Str(Resources.SCM.FlowMaster.FlowMaster_ResourceGroup, true, false, l));
             str = str.Append(Str(Resources.SCM.FlowMaster.FlowMaster_CreateHuOption, true, false, l));
             ...
             str = str.Append(Str(Resources.SCM.FlowDetail.FlowDetail_Machine, true, false, l));
             ...
             foreach (var flowDetail in flowDetails)
             {
                 ...
                 str = str.Append(Str(flowDetail.CurrentFlowMaster.OrderDeviation, false, false, l));
                 str = str.Append(Str(systemMgr.GetCodeDetailDescription(CodeMaster.CodeMaster.ResourceGroup, (int)flowDetail.CurrentFlowMaster.ResourceGroup), false, false, l));
                 str = str.Append(Str(systemMgr.GetCodeDetailDescription(CodeMaster.CodeMaster.CreateHuOption, (int)flowDetail.CurrentFlowMaster.CreateHuOption), false, false, l));
                 //str = str.Append(Str(flowDetail.CurrentFlowMaster.CreateHuOption, false, false, l));
                 ...
                 str = str.Append(Str(flowDetail.Machine, false, false, l));
