...
{
    public class MachineController : WebAppBaseController
    {
        ...
        private static string selectCountInstanceStatement = "select count(*) from MachineInstance as m";

        private static string selectInstanceStatement = "select m from MachineInstance as m";

        //
        // GET: /ProdLineEx/

        #region MachineInstance
        [SconitAuthorize(Permissions = "Url_MRP_MachineInstance_View")]
        ...
        public ActionResult _AjaxInstanceList(GridCommand command, MachineInstanceSearchModel searchModel)
        {
            ...
            return PartialView(GetAjaxPageData<MachineInstance>(searchStatementModel, command));
        }
        #region 导出后加工日历
        [SconitAuthorize(Permissions = "Url_MRP_MachineInstance_View")]
        public void ExportXLS(MachineInstanceSearchModel searchModel)
        {
            ...
            ExportToXLS<MachineInstance>(fileName, GetAjaxPageData<MachineInstance>(searchStatementModel, command).Data.ToList());
        }

        #endregion
        [HttpGet]
        [SconitAuthorize(Permissions = "Url_MRP_MachineInstance_View")]
        public ActionResult InstanceEdit(string code, string dateIndex, string dateType)
        {
            if (string.IsNullOrEmpty(code) || string.IsNullOrEmpty(dateIndex) || string.IsNullOrWhiteSpace(dateType))
            {
                return HttpNotFound();
            }
            else
            {
                MachineInstance machineInstance = this.genericMgr.FindAll<MachineInstance>("select m from MachineInstance as m where m.Code=?  and m.DateIndex=? and  m.DateType=?", new object[] { code, dateIndex, dateType })[0];
        ...
        [SconitAuthorize(Permissions = "Url_MRP_MachineInstance_View")]
        public ActionResult InstanceDelete(string code, string dateIndex, com.Sconit.CodeMaster.TimeUnit dateType)
        {
            if (string.IsNullOrEmpty(code) || string.IsNullOrEmpty(dateIndex))
            {
                return HttpNotFound();
            }
            else
            {
                MachineInstance machineInstance = this.genericMgr.FindAll<MachineInstance>("  from MachineInstance as m where m.Code=? and m.DateIndex=? and  m.DateType=?",
                         new object[] { code, dateIndex, dateType })[0];
        ...
        public ActionResult New(Machine machine)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    Machine machineExist = genericMgr.FindAllIn<Machine>("from Machine as i where i.Code=?  ", new object[] { machine.Code }).FirstOrDefault();
                    if (machineExist != null)
                    {
                        SaveErrorMessage("Machine code diplicate");
                        return View(machine);
                    }
                    ...
                    return new RedirectToRouteResult(new RouteValueDictionary { { "action", "Edit" }, { "controller", "Machine" }, { "id", machine.Id } });
                }
            }
            catch (Exception e)
            {
            ...
            , bool isQty, bool isIslandQty, bool isShiftType, bool isMachineType, int checkboxcheckedCount)
        {
            ...
            string hql = "  from MachineInstance as m where m.DateType=? and  DateIndex between ? and ? ";
            param.Add(dateType);
            ...
            IList<MachineInstance> machineInstanceList = genericMgr.FindAll<MachineInstance>(hql, param.ToArray());
        ...
        private string GetStringMachineInstance(IList<MachineInstance> machineInstanceList, com.Sconit.CodeMaster.TimeUnit DateType,
